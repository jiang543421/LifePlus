...
- [ ] **Step 1: V3** with columns + `idx_user_start` per `02-database §3`.
- [ ] **Step 2: `flyway:info` confirm, commit** `feat(db): t_plan`

---

## Task T-P02: `Plan` entity + `PlanMapper`

- [ ] **Step 1: IT** insert, find by id, page by user+range (uses `idx_user_start`).
- [ ] **Step 2: Implement** entity + mapper.
- [ ] **Step 3: Run, commit** `feat(plan): plan entity and mapper`

---

## Task T-P03: `PlanService` + cross-user check + range query

- [ ] **Step 1: Unit tests** for range query, cross-user (1003), `endTime > startTime` (1001).
- [ ] **Step 2: Implement**, commit `feat(plan): plan service`

---

## Task T-P04: `PlanController` 5 endpoints

- [ ] **Step 1: Slice tests**, implement DTOs and controller matching `03-api-auth §5.4`.
- [ ] **Step 2: Run, commit** `feat(plan): plan controller`
...
- [ ] **Step 1: Test, run, commit** `test(plan): e2e with task link`
...
- [ ] **Step 1: Test, implement** wrapping endpoints, commit `feat(frontend): plan api`
...
- [ ] **Step 1: Test** `CalendarMonth` renders day grid + event dots.
- [ ] **Step 2: Implement**: month grid via dayjs; click day → side panel list.
- [ ] **Step 3: Run, commit** `feat(frontend): calendar month view`

---

## Task F-P04: `PlanDetailView` + `EventDialog`

- [ ] **Step 1: Test** dialog open/close + save.
- [ ] **Step 2: Implement** event dialog (create/edit), detail read-only.
- [ ] **Step 3: Run, commit** `feat(frontend): plan detail and dialog`

---

## Task F-P05: Calendar e2e

`frontend/e2e/plan.spec.ts`: create plan on day X → calendar marker on day X.

- [ ] **Step 1: Write, run, commit** `test(frontend): plan e2e`
