...
**全部既有**：Spring Security、`@Cacheable`、RedisTemplate、UserContext、TaskMapper/PlanMapper/ExpenseMapper/DietMapper、DailyMetricProvider（复用）、Element Plus、Pinia、Axios。

### 4.4 架构边界

- **不依赖** `daily.web` / `daily.service.DailyReportService`（那是日报业务，不是 AI 输入）
- 仅**调用** `daily.provider.DailyMetricProvider` 作为数据源
...
| 方法 | 路径 | 说明 | 鉴权 | 限流 |
|---|---|---|---|---|
| GET | `/api/ai/insight/today` | 返回当前洞察（缓存优先） | access token | 60 次/分/IP |
| POST | `/api/ai/insight/refresh` | 清缓存 + 立即重算 | access token | 6 次/分/IP |
...
| `freshnessSeconds` | int | ✅ | 在 `AiAnalysisController` 层现算：`Duration.between(generatedAt, now()).getSeconds()`，前端展示"X 分钟前"；负值钳为 0 |
...
### T5：Service 编排
- **T5.1** `AiAnalysisService.getOrCompute(userId)`：缓存 → provider 循环 → 模板 → 回写
...
- **T6.2** `AiAnalysisControllerWebTest`（6 个用例：401/200/503/refresh）
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| `DailyMetricProvider` 接口未提供 `computeStreak` | T4.6 阻塞 | 实现前先在 `daily` 包加 `computeStreak(userId): MetricValue`，独立 commit |
