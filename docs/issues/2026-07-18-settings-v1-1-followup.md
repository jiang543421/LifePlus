...
## 6. 2026-07-21 verification（HIGH-2 / HIGH-3 当前实现核对）

按 v1.2.3 backlog 节奏，R-004 / R-006 收尾后核对 HIGH-2 / HIGH-3 仍生效：

### HIGH-2：access token deny-list（CLOSED via TTL 缩短）

| 维度 | 内容 |
|---|---|
| `application.yml:29-32` | `access-ttl: PT15M` + deny-list 决策 trade-off 注释（已落地） |
| `AuthServiceIT.java:81-83` | `assertThat(loginResp.expiresIn()).isEqualTo(jwtProperties.getAccessTtl())` — 避免返回的 expiresIn 与 token 真实寿命不一致 |
| 替代 deny-list 的攻击面 | refresh 旋转（7d）+ 前端 auth.clear() 改密后立即清空 |
| **2026-07-21 mvn verify** | 314 surefire + 5 AuthServiceIT（含 expiresIn 断言）+ 0 fail ✓ |

### HIGH-3：密码策略收紧（CLOSED via settings v1.1 PR）

| 维度 | 文件 |
|---|---|
| 策略常量（MIN/MAX/复杂度正则/70+ 项弱密码字典） | `backend/src/main/java/com/lifepulse/auth/security/PasswordPolicy.java` |
| Bean Validation 注解 | `backend/src/main/java/com/lifepulse/auth/security/StrongPassword.java` |
| Validator（统一错误码 1001 文案） | `backend/src/main/java/com/lifepulse/auth/security/PasswordPolicyValidator.java` |
| DTO 联动 | `RegisterRequest` / `ChangePasswordRequest`（@StrongPassword） |
| 后端单测（22 用例覆盖长度 / 复杂度 / 字典） | `backend/src/test/java/com/lifepulse/auth/security/PasswordPolicyValidatorTest.java` |
| DTO 边界单测（11 用例） | `backend/src/test/java/com/lifepulse/auth/dto/DtoValidationTest.java` |
| 前端规则镜像 | `frontend/src/types/index.ts:65-74` `PASSWORD_RULES`（length/letter/digit/weak） + `WEAK_PASSWORDS` Set |
| 前端 UI 联动 | `frontend/src/views/SettingsView.vue` 三卡片 + `frontend/src/components/PasswordRules.vue` |
| 前端测试 | `frontend/src/views/__tests__/SettingsView.spec.ts` |
| **2026-07-21 mvn verify** | PasswordPolicyValidatorTest 22/22 + DtoValidationTest 11/11 + AuthServiceTest 16/16 + 314 surefire 全量 + AuthServiceIT 5/5 + 0 fail ✓ |

### 结论

HIGH-2 / HIGH-3 闭环完整，2026-07-21 mvn verify 全绿。settings v1.1 follow-up backlog 全消化（MEDIUM 1002 决策保留 1002；INFO entity mutation 留待全局重构）。

后续 backlog 候选：
- E2E el-date-picker / el-select helper 改造（T18+ Playwright security.spec.ts 依赖此前置）
- 日报模块 v1.2.3
- R-004 Playwright E2E 落地（同依赖 helper）

---

**Owner**: lp
**Created**: 2026-07-18
**Last updated**: 2026-07-21（HIGH-2 / HIGH-3 verification on current main）
